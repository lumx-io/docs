...
You can verify a beneficiary’s status by using the [API reference](/api-reference/customers/read-a-customer-verification).

For business customers with beneficiaries, each beneficiary will have their own verification status that can be checked individually.

<Note>
  Business verification typically takes longer than individual verification due
  to the additional due diligence required for both the business entity and its
  beneficiaries.
</Note>
